...
import { assembleLifeOSTestPrompt } from '@/lib/knowledge';
...
/**
 * Capability-enhanced simulation endpoint.
 * Uses assembleLifeOSTestPrompt() which includes:
 * - Council directives as LifeOS capability definitions (identity dissolved)
 * - LifeOS system voice directive
 *
 * POST /api/simulation/test
 * Body: { vignette_id, mode?, messages?, additional_context? }
 */
export async function POST(request: Request) {
  ...
  const systemPrompt = assembleLifeOSTestPrompt({
    mode: simulationMode,
    vignette,
